...
def consume_item(hero: list, idx: str) -> None:
    ...
    if idx <= len(hero[10]) - 1 and idx > -1:
        ...
        if hero[10][idx] == "зелье здоровья":
            ...
            if hero[1] > hero[2]:
                hero[1] = hero[2]
            print(f"{hero[0]} востановил здоровье") # TODO: показать, сколько очков здоровья увеличелось
        elif hero[10][idx] == "зелье силы":
            hero[6] += 1
            print(f"{hero[0]} прибавил атаку на 1")
        else:
...
def visit_hub(hero: list) -> None:
    ...
    input("\nНанжите ENTER чтобы продолжить из х")


def visit_shop(hero: list):
    text = f"{hero[0]} зашёл в лавку алхимика. Здесь продаются зелье!"
    options =[
        "Купить зелье здоровья за 10",
        "Купить зелье силы за 20",
        "Уйти в хаб"
    ...
    if option == 0:
       ...
       return visit_hub(hero)
        ...
        print("Такой вариант ещё не сделан")
        return visit_hub(hero)
    input("\nНанжите ENTER чтобы продолжить из м")


def visit_inn(hero: list):
    text = f"{hero[0]} зашёл в в казино!"
    options = [
    "Начать играть",
    "Выйти в хаб"
    ]
